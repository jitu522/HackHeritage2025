...
def init_assign_courses_routes(app):
    ...
    def assign_courses():
        ...
        slot_minutes = int((end_time.hour * 60 + end_time.minute) -
            ...
            for teacher, code, session_time, sessions_needed, is_lab in course_list:
                ...
                while booked < sessions_needed and attempts < max_attempts:
                    ...
                    for day in sorted_days:
                        ...
                        if is_lab and random.choice([True, False]):
                            morning_slots = [s for s in slots_for_day if s.hour < 12]
                            afternoon_slots = [s for s in slots_for_day if s.hour >= 12]
                        ...
                        for i in range(0, max(0, len(slots_for_day) - required_slots + 1)):
                            ...
                            for j in range(len(consecutive) - 1):
                                diff = ((consecutive[j + 1].hour * 60 + consecutive[j + 1].minute) -
                                        (consecutive[j].hour * 60 + consecutive[j].minute))
        ...
        cur.execute(f"""
            SELECT day, slot_start, teacher_name, course_code, is_lab, time_slot
            FROM {routine_table}
            WHERE is_lab = TRUE
            ORDER BY day, slot_start
        """)
        lab_slots = cur.fetchall()
        ...
        for day in active_days:
            cur.execute(f"""
                SELECT slot_start, time_slot
                FROM {routine_table}
                WHERE day = %s
                ORDER BY slot_start
            """, (day,))
            day_slots = cur.fetchall()
            break_times = [t[0] for t in day_slots if t[1] == "BREAK"]
            break_time = break_times[0] if break_times else None

            for lab in lab_slots:
                if lab[0] == day:
                    pass

        # TODO: full moving logic would be implemented here
        # (Due to space, I can paste final complete function in follow-up if needed)
